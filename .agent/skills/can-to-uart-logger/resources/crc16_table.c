 ...
 * This table is used for high-performance CRC calculation in the CAN to UART logger.
 */
static const uint16_t crc16_table[256] = {
	...
	0xB98A, 0xA9AB, 0x5844, 0x4865, 0x7806, 0x6827, 0x18C0, 0x08E1, 0x3882, 0x28A3, 0xCB7D, 0xDB5C, 0xEB3F, 0xFB1E, 0x8BF9, 0x9BD8, 0xABB1, 0xBB90,
...
/**
 * Calculate CRC16 (CCITT-FALSE) using the lookup table.
...
{
	uint16_t crc = 0xFFFF;
	for(uint16_t i = 0; i < len; i++) { crc = (crc << 8) ^ crc16_table[((crc >> 8) ^ data[i]) & 0xFF]; }
	return crc;
}
