...
extern "C"
{
	...
	*/
	...
	void CAN1_CallbackRxBufferOverflow(void);
	...
	/**
	  ...
	  @Returns
			...
			CAN_TX_MSG_REQUEST_SUCCESS          - Transmit message object
	  successfully placed into Transmit Buffer CAN_TX_MSG_REQUEST_DLC_ERROR        -
	  Transmit message object DLC size is more than Transmit Buffer configured DLC
	  size CAN_TX_MSG_REQUEST_BUFFER_FULL      - Transmit Buffer is Full
	...
	*/
	...
	/**
	  ...
	  @Preconditions
			CAN1_Initialize() function should be called before calling this
	  function. The CAN1_ReceivedMessageCountGet() function should be checked to see
	  ...
	  @Example
			...
			//      The user must have to allocate 8 bytes of space for CAN1 Receive
	  message object data.
	...
	*/
	...
	/**
	  ...
	  @Description
			...
			If Receiver error counter is above 95 to below 128, then receiver error
	...
	*/
	...
	/**
	  ...
	  @Description
			...
			If Receiver error counter is above 0 to below 128, then receiver error
	  active state is set.
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine checks whether the transmitter is in the error passive
	  state. If Transmitter error counter is above 127, then the transmitter error
	  passive state is set.
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine checks whether the transmitter is in the error warning
	  state. If Transmitter error counter is above 95 to below 128, then transmitter
	  error warning state is set.
	  ...
	  @Example
			<code>
			//Note: Example code here is not based on MCC UI configuration,
			//      sample code to demonstrate CAN transmit error warning APIs
	  usage.
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine checks whether the transmitter is in the error active
	  state. If Transmitter error counter is above 0 to below 128, then transmitter
	  error active state is set.
	  ...
	  CAN1_OperationModeSet(CAN_NORMAL_2_0_MODE))
							{
									...
									{
											if(CAN_TX_FIFO_AVAILABLE ==
	  (CAN1_TransmitFIFOStatusGet(CAN1_TX_FIFO1) & CAN_TX_FIFO_AVAILABLE))
											{
													CAN1_Transmit(CAN1_TX_FIFO1,
	  &msg);
											}
									}
							}
	...
	*/
	...
	/**
	  @Summary
			CAN1 Return the number of messages object that are received in RX
	  buffer.
	  ...
	  CAN1_OperationModeSet(CAN_NORMAL_2_0_MODE))
							{
									...
									if(CAN_DISABLE_MODE == CAN1_OperationModeGet())
									{
											...
											C1INTLbits.WAKIF = 0; // Clear Wake-Up
	  interrupt flag
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine configures the CAN1 Transmit DMA buffer and DMA buffer
	  configuration is mandatory forCAN1 2.0 module. Before calling CAN1_Transmit
	  function, CAN1_TransmitEnable function has to be called.
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine configures the CAN1 Receive DMA buffer and DMA buffer
	  configuration is mandatory forCAN1 2.0 module. Before calling CAN1_Receive
	  function, CAN1_ReceiveEnable function has to be called.
	...
	*/
	...
	/**
	  @Summary
			Assigns a function pointer with a CAN1 Transmit Error State BusOff
	  callback address.

	  @Description
			This routine assigns a function pointer with a CAN1 Transmit Error State
	  BusOff callback address.
	...
	*/
	...
	/**
	  @Summary
			Assigns a function pointer with a CAN1 Transmit Error State Bus Passive
	  callback address.

	  @Description
			This routine assigns a function pointer with a CAN1 Transmit Error State
	  Bus Passive callback address.
	...
	*/
	...
	/**
	  @Summary
			Assigns a function pointer with a CAN1 Receive Error State Bus Passive
	  callback address.

	  @Description
			This routine assigns a function pointer with a CAN1 Receive Error State
	  Bus Passive callback address.
	...
	*/
	...
	/**
	  @Summary
			Assigns a function pointer with a CAN1 Bus Wake-Up activity callback
	  address.

	  @Description
			This routine assigns a function pointer with a CAN1 Bus Wake-Up activity
	  callback address.
	...
	*/
	...
	/**
	  ...
	  @Description
			This routine assigns a function pointer with a CAN1 Receive callback
	  address.
	...
	*/
	...
	/**
	  @Summary
			Assigns a function pointer with a CAN1 Receive Over Flow callback
	  address.

	  @Description
			This routine assigns a function pointer with a CAN1 Receive Over Flow
	  callback address.
	...
	*******************************************************************************/
	...
	CAN1_receive(uCAN_MSG *recCanMsg);
	 ...
	 * transmitted sendCanMsg: pointer to the message object Return Value:      true
	 * - Transmit successful false - Transmit failure
	 ******************************************************************************/
	bool __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_Transmit instead.")))
	CAN1_transmit(CAN_TX_PRIOIRTY priority, uCAN_MSG *sendCanMsg);
	...
	uint8_t __attribute__((deprecated("\nThis will be removed in future MCC releases. "
									  "\nUse CAN1_ReceivedMessageCountGet instead.")))
	CAN1_messagesInBuffer();
	...
	void __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_DefaultBusErrorHandler instead.")))
	CAN1_CallbackBusOff(void);
	void __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_DefaultTxErrorPassiveHandler instead.")))
	CAN1_CallbackTxErrorPassive(void);
	void __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_DefaultRxErrorPassiveHandler instead.")))
	CAN1_CallbackRxErrorPassive(void);
	void __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_DefaultReceiveBufferHandler instead.")))
	CAN1_CallbackMessageReceived(void);
	void __attribute__((deprecated("\nThis will be removed in future MCC releases. "
								   "\nUse CAN1_DefaultRxBufferOverFlowHandler instead.")))
	CAN1_CallbackRxBufferOverflow(void);
#ifdef __cplusplus // Provide C++ Compatibility
}
